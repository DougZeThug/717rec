...
**Why this file exists.** A handful of production-critical switches live _outside
the repo_ — in the Supabase Dashboard, Lovable, Cloudflare, and the domain
...
- [`OPERATIONS.md`](OPERATIONS.md) — the league-night "what do I do when X breaks"
  playbook, recovery steps, and the merge gate. This file is the _settings_; that
  file is the _actions_.
...
> _intentions_. The **Last verified** column is different: it can only be filled
...
| Secure email change           | Authentication → Sign In / Up → Email | **ON**         | Requires confirmation on _both_ the old and new address before an account's email is changed. Cheap protection against an account being quietly re-pointed. On regardless of the confirmations policy above.                                                                                                                                                                                                                    | _needs DW_    |
...
> form has its _own_ app-level limit — **5 submissions per 10 minutes per IP**,
...
> Login, magic-link, and password-reset redirects break on _any deployed surface_
...
| Setting               | Where                                   | Expected value                                                                                   | Why                                                                                                                                                                                         | Last verified |
| --------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| Custom SMTP           | Authentication → Emails → SMTP Settings | Record which: **custom SMTP** (note provider + sender address) **or** Supabase's built-in sender | Supabase's built-in sender is rate-limited and best-effort — fine for our volume, but you must _know_ which one is live so an undelivered reset email isn't a surprise.                     | _needs DW_    |
...
Dashboard → **Database → Backups**. The step-by-step _restore_ procedure lives in
...
| Restore button located        | Database → Backups | You've _seen_ where **Restore** is             | Finding the button for the first time mid-incident is how mistakes happen.                                                                                           | _needs DW_    |
...
| `RESEND_API_KEY`            | `send-support-email`, `submit-contact-request` (both via `_shared/email.ts`) | yes if admin email alerts are used                  | Edge Functions → Secrets | Without it, both functions silently skip the send step and report `emailed: false`. The message is still stored and still shows in the admin Contact Inbox, so nothing is lost.                                                                                                                                                   | _needs DW_    |
| `ANTHROPIC_API_KEY`         | `generate-recap-caption`                                                     | only if AI recap captions are used                  | Edge Functions → Secrets | Writes the draft caption for the Weekly Content Pack. Without it the function returns **503** with `code: caption_unconfigured` and the admin screen says captions are not set up — graphics, preview and publishing all still work, and the caption can be written by hand. Server-only; never expose it in a `VITE_*` variable. | _needs DW_    |
...
_Baseline written 2026-07-23. First full dashboard verification: ***needs DW***._
