...
| The connection | With no signal the End Game button is greyed out and a line above it says the game cannot be ended until the signal comes back. | Losing the signal greys the button. Reconnecting does not hand it straight back: it stays greyed, now naming the round still being filed, until that round settles. |
...
and the score; only the writing waits.

**A signal coming back is not enough on its own.** The browser reports a
connection the moment it has one, while the round it was holding is still on its
way and can still be refused. So the button stays greyed out for that gap too,
under "Waiting for the last round to be filed — the game cannot be ended until
it is", and frees up once the round has settled. The same line shows for the
moment an ordinary online round save is in flight. This is deliberate rather than a
