...
| FRESH-03 | P1 | offline | Outside live scoring there is no offline write queue; a write offline fails and is lost ([Offline](../foundations/saving-and-freshness.md#offline)). | Signed in, any form other than live scoring. | 1. Go offline.<br>2. Fill in a form and submit.<br>3. Go back online and wait five minutes. | The submit fails with a message. Nothing is sent when the connection returns. See ERR-11 for the live-scoring exception. | — |
