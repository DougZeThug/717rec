...
| ID | P | Device | Claim | Setup | Steps | Expected | Result |
| --- | --- | --- | --- | --- | --- | --- | --- |
| MSG-01 | P1 | browser | Only one toast is shown at a time; a second replaces the first ([Toasts](../foundations/messages-to-the-user.md#toasts)). | Anything that can raise two toasts close together, e.g. bulk admin score entry. | 1. Trigger two messages within a second.<br>2. Count what is on screen. | One toast. The first is gone. **(suspected bug)** See [`admin/enter-scores-in-bulk.md`](../admin/enter-scores-in-bulk.md). | — |
