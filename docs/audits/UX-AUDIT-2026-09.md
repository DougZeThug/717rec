...
- **Still open between 1024 and ~1180 px (measured during W11).** Q1 moved the desktop link row to `lg:`, which fixed 768–1023. It did not fix the band above it: the row needs about 1244px and the container at 1024 gives 992, so the content overflows by 96–252px and the theme toggle sits off the right edge of the window — the original symptom, at a different width. The row only fits from about 1280. W11 took 108px out of it (icon-sized search trigger, `px-3` links) but cannot close a 252px gap; that needs the "More menu" half of this finding's recommendation, or fewer links. **axe `button-name` on the header is fixed** — the search trigger had no accessible name below `xl`, and now has one at every width.
...
- **The trigger is icon-sized now, and the row is tighter.** Its 15rem expanded form from `xl` was built for the pill bar, which gave it a whole flex column. The header row has about 96px of slack, so a 240px control overflowed it at every desktop width and the shell's `overflow-x-hidden` swallowed the evidence (found by review). It is a 36px search icon with the shortcut in its tooltip, and the nav links dropped from `px-4` to `px-3`, which is 72px the row could not spare. Signed out, the row now fits at 1280 where it overflowed by ~56px before this PR.
- **X-01 is not as closed as it looks — see the note under it.** Measuring for the above found the header still clipping between 1024 and ~1180.
...
- **Done (W1 + W2).** The phone menu is one bar: a "Sections" button naming the section on screen, plus the two Quick Access buttons. The search box and the six groups moved into a drawer behind it, so a section starts near the top of the page. `aria-current` is set on the active item, the phone section list is a named "Admin sections" landmark like the sidebar, and the open group follows the section in the address.
- **One deviation: the bar is not sticky.** `Navbar.tsx` is already `sticky top-0 z-50`, so a second sticky bar slides underneath it; making it work needs the navbar's real height as a variable, which does not exist and varies with the breakpoint and the iOS safe-area inset. The finding is about first-screen space, which one short non-sticky bar solves on its own.
...
- **Done (W5).** A bar under the header says the connection is gone, and says it is back for a few seconds when it returns. A page whose code never downloaded gets its own short screen — "This page did not download" — instead of the developer error, and it opens itself the moment the signal returns, with nothing pressed. `useOnlineStatus` (`src/hooks/useOnlineStatus.ts`) is the one source of that state for both.
  - **The banner is not sticky**, for the reason recorded under X-06: `Navbar.tsx` is already `sticky top-0 z-50`, so a second sticky bar slides underneath it. It sits in normal flow directly below the header, pushes the page down rather than covering it, and scrolls away once read. It carries `role="status"`, never `role="banner"` — that would be a second landmark on every page, which X-08 exists to prevent.
  - **Correction to the finding's premise.** It says to "re-import on reconnect". That is not available: React 18's `React.lazy` caches a *rejected* import for the life of the document and re-throws the stored error on every later attempt. That is also the real reason the old "Try Again" button looked broken — it reset the boundary's state and the same error came straight back. Recovery is the document loading again, which is the only thing that clears that cache, and it only fires once the connection is back so it cannot land on the browser's own error page.
  - **The boundary had to move, and this was proven in the browser, not read off the code.** The finding says the route-level `Suspense` has no failure path, and every route element already sat inside a `RouteErrorBoundary` — so the fix looked like a one-line branch there. It is not: when a lazy import rejects, React re-throws it from the **`Suspense` boundary's own position**, and `Suspense` wraps `Routes`, so the per-route boundaries are below it and never in the path. The app-level `ErrorBoundary` caught it instead and took the header with it — exactly the screenshot in this finding. A first `e2e/offline.spec.ts` run reproduced it. There is now a boundary directly above the `Suspense` in `App.tsx`; the per-route ones are unchanged. The app-level boundary learned the same branch as a backstop, and `path="*"` — the one route with no boundary of its own — gained one.
  - **Two smaller things fell out of it.** A failed page download is no longer reported to Sentry as a crash: `sentry.ts` only dropped `'loading chunk'`, which is the older bundler's wording, and this build is Vite, which words it three other ways. And `routePrefetch.ts`'s comment that "the route error boundary owns a genuine failure" is true for the first time.
  - **The query client's online state is not the browser's, and had to be taught.** `onlineManager` hardcodes itself to `true` and its own setup only registers `online`/`offline` listeners — it never reads `navigator.onLine`. An app opened or *restored* while already offline therefore believed it had a connection, and since no transition ever fires there was nothing to correct it: no banner, and (for W6) a round sent down the failing save path rather than parked. `initOnlineStatus()` (`src/utils/onlineStatus.ts`) seeds it at startup through the library's own `setEventListener`, so the seed is re-applied if the listener is ever installed again. Found by a review bot, not by us — worth knowing before anything else is built on that manager.
  - **The reload is allowed once per half-minute, not once per component.** A ref cannot hold "we already tried": the reload builds a new component with a fresh one, so a file that is genuinely gone — a deploy that removed it, a CDN that keeps failing — would have reloaded, failed and reloaded endlessly. The marker is in session storage, and after that one attempt the reader gets the panel and the button rather than another silent reload. A later, genuine incident in the same visit is minutes away and gets its own attempt.
  - **Still not covered.** Nothing is disabled while offline and no form warns before it is filled in; the bar explains a failure rather than preventing one. It also reads the browser's own answer, which counts a captive portal as online.
...
- **Partly done (Q25).** The labels are one vocabulary now: the queue is "Score approvals" in the menu, the section heading and the League Night tile; one report is a "Score report"; "Deny" is gone; the Match Creation section is named that in every heading. `MATCH_STATUS_LABELS` (`src/types/matchStatus.ts`) was added here for the two exceptional states, and L1 widened it to all four. The realtime pill was already renamed by Q19 — "Live updates: on / connecting / off" rather than the "Connected / Reconnecting" suggested here. The `deriveMatchStatus()` unification is **L1**, done — with one correction to this finding.
  - **Done (L1).** `deriveMatchStatus()` (`src/utils/matchStatus.ts`) returns exactly one of `completed`, `canceled`, `postponed`, `scheduled`, and `MATCH_STATUS_LABELS` now covers all four. The Schedule card and tabs, Home's "my matches" and next-match picker, the score queue, mass score entry, Live Corrections and the live-scoring gate all read it. A finished match is "Final" everywhere: "Completed", "Complete", "finalized" and "Match complete" are gone from those screens.
  - **Correction to this finding.** It says six components derive status from different fields. The real count is about seventy sites over nine rules, and the fix was scoped to the screens and queues a person sees. The `.eq('iscompleted', true)` filters in stats, career, weekly recap and the MCP tools were deliberately left alone: they are correct, and they feed standings and Power Score.
  - **Correction to L1's premise.** L1 says the model should "exclude postponed/canceled from queues". It could not: **the `matches` table has no `status` column**, nothing writes `Match['status']`, and no screen can set it, so the three places that read it were dead branches. The exclusion is wired — every queue and scoring gate asks `isMatchOpenForScoring()` — but it cannot fire until that column exists. Adding it is a separate change (migration plus an admin control).
  - **A bug found on the way, fixed.** `ScheduleContent.tsx` compared the nullable `iscompleted` strictly against a boolean, so a match with no recorded result matched neither tab and was visible to nobody. `Match['iscompleted']` is typed `boolean | null` now, and `null` reads as "not finished" everywhere.
...
- **Done (W4).** `/schedule?date=&q=`, `/stats?view=all`, `/teams/<team>#matches`, and Home's "my match" row now points at `/schedule?date=…#match-<id>`. Playoffs was already done by Q23.
- **Deliberately left out of the address.** Schedule's tab, its expanded groups, the standings sort and expanded row, and how far a page is scrolled. Each is derived or ephemeral, and each would need a rule for what happens when it disagrees with a smart default — which is the failure mode CP-01 and SC-01 were. Schedule's tab is the clearest case: it is already chosen to suit the night, so a link to a past night opens on Completed by itself.
- **Three details that were not obvious.** Schedule's date round-trips as local midnight, never through `new Date(string)`, or the night shifts by one in timezones behind UTC. A date in the address counts as chosen, so the SC-01 "move off an empty guess" correction leaves a shared link alone. The team page reads its section once on arrival, because the section bar writes the address as the reader scrolls and re-reading it would reopen sections under them.
- **T-03's other half.** Match History opens by default now. A hash naming one of the other sections opens it too. `/schedule#match-<id>` scrolls to a card but does not switch tabs to find one that is not on screen.
...
- **H-02 · Two contact forms with different purposes — Medium.** The home form ("Request a timeslot change, report a score, join the league… Got a bug? Use the Contact page instead") and `/contact` ("Need a timeslot change…? Use the message form at the bottom of the home page instead") each redirect users to the other (`anon/home--m390.jpg`, `anon/contact--m390.jpg`). B-10 merged the *inbox*; the *entry* is still split, with different field sets (home: request type + team; contact: subject select). *Recommend:* one `/contact` form with a "What is this about?" select whose options are the union (timeslot, score, join, bug, other); replace the home form with a compact card linking to it. *Effort:* M. *Accept:* one form, one inbox, no cross-referral copy.
  - **Done (W7).** `/contact` opens with **"What is this about?"** and seven options — the union of the two old vocabularies. The answer decides which fields appear: Join the league asks for a proposed team name and who else is playing; the topics answered by email ask for an email rather than "email or phone". The home page carries a card with one button instead of a form, keeping the `contact-panel` anchor so old links still land. Neither page tells anyone to use the other any more.
  - **One form, still two mailboxes — deliberately.** The acceptance line says "one inbox". The *reader's* half of that is done and the admin's was already done by B-10, which put both in one list. What is not merged is the two **stores**: `contact_requests` and `support_tickets` have different columns, different RLS (support has no INSERT or DELETE policy at all), different edge functions and different rules — the league one verifies who is asking against their signed-in profile and can be deleted, the support one emails the admins and cannot. Merging them is a migration plus an edge-function rewrite plus an admin-inbox rework, which is a bigger and riskier change than this finding is asking for, and nothing a reader can see depends on it. The topic routes the message; `contactTopics.ts` is the one place that mapping lives.
  - **Two rules had to fork, and are named on screen rather than hidden.** The support mailbox answers by email, so those topics ask for an email address and say so when a phone number is given ("this one is answered by email"); the league mailbox still takes either, which a rec league wants. The two edge functions also accept different message lengths (2,000 and 5,000), so the limit follows the topic instead of being cut to the lower of the two.
  - **`?type=` works now**, which is what **H-01** asked for: `/contact?type=join_league` opens on Join the league with its fields showing. It is read once on arrival and never written back — after the first paint the picker belongs to the reader.
  - **Three tests moved rather than being deleted with the old form.** The home panel's cases for deriving a signed-in member's name and team *during render* rather than copying them in an effect are the rule that keeps a typed name from being overwritten when the profile arrives late. They live in `ContactForm.test.tsx` now.
  - **`e2e/toast-messages.spec.ts` needed reworking, not just re-selecting.** Its "three toasts at once" case pressed the home form's empty-submit guard, which toasted; the surviving form reports a bad field under the field. It raises the three toasts from three refused sends instead — which is what a real user hits at the league's rate limit, and a better test than the one it replaces.
...
- **H-04 · Second `h1`, redundant alt text — Low.** axe `page-has-heading-one` passes but Home has two `h1`s (hero title + SEO title); champions logos repeat the team name in `alt` (`image-redundant-alt` ×6). *Recommend:* make the SEO heading an `h2`/`p`; `alt=""` on decorative logos next to text. *Effort:* S. **Correction (Q12):** there is no SEO heading — `SeoHead` writes only `<head>` tags. The two `h1`s are the `md:hidden` and `hidden md:block` hero variants, so `display:none` keeps one of them out of the accessibility tree and exactly one is ever exposed. No change was made; the `alt` half stands.
...
- **T-02 · Sort control exists only on mobile; view controls persist in localStorage but the division toggle on Stats does not — Low (consistency).** `useTeamsPreferences.ts` persists three prefs; Standings' Division/All resets. *Recommend:* same persistence for both (URL param preferred, see X-14). *Effort:* S.
  - **Half done (W4).** Division/All is in the address now (`/stats?view=all`), which is the preferred half. The sort is still forgotten on every visit.
- **T-03 · Team page sections cannot be linked and Match History is collapsed by default — Medium.** `TeamDetailsStickyNav.tsx:94-114` scrolls without writing a hash; the section a player most wants (matches) needs a tap (`anon/teams-3-amigos--m390.jpg`). *Recommend:* open Match History by default; write `#matches` to the URL and honour it on load. *Effort:* S. *Accept:* `/teams/3-amigos#matches` opens with the list expanded and scrolled into view.
  - **Done (W4).** Match History opens by default, and the section bar writes `#matches`. Any of the five sections can be named: the page opens that one and scrolls to it, once the team has loaded. The section is read once on arrival — the bar rewrites the address as the reader scrolls, and re-reading it would reopen sections under them.
...
- **Partly done (W8).** A chip row under the search box: **All** plus one per division, and **My team** for a signed-in member with an approved team. Both are in the address (`?division=intermediate&team=mine`), so a filtered week can be shared and survives a reload. The active chip wears its own division colour, from the same `getDivisionBadgeColor` the timeslot cards already use.
  - **The season half is not done.** Past seasons are still unreachable from Schedule, because `fetchScheduleMatches` is scoped to the active season in the query itself (`MatchScheduleAdminService.ts`). A season filter is a different change — a new fetch, not a new predicate — so this row is marked partly done rather than done.
  - **One chip per *display* division.** The league runs "Competitive High" and "Competitive Low"; those are one **Competitive** chip. Matching is on `division_id` for matches and on the division's name for timeslot rows — whichever the row actually carries — and never on `getDisplayDivision()`, which falls back to "Recreational" for a name it does not know and would have filed every unrecognised division under one chip.
  - **Either side counts.** Cross-division matches are real, and a Competitive team's own match must not vanish from the Competitive chip because its opponent is Intermediate.
  - **All three tabs, not two.** The chips were nearly limited to Upcoming and Completed, which share one filtered list; Timeslots is fed by a different query. But timeslot rows carry the team and the division name, so filtering them is a filter over the grouped record — and showing a control on one tab that another quietly ignores is worse than not having it. A time whose every row is filtered out is dropped, and the Timeslots tab grew its own "no matches match these filters" state so a chip is never reported as an empty league night.
  - **`team=mine`, not a team id.** A link shared by one member shows the reader their own team, or nothing. It never leaks whose link it was.
  - **The chips are not a `ToggleButtonGroup`.** They started as one. Its pills variant gives every option an equal share of one row, and at 390 px four options clipped "Recreational" — which is the defect T-01 and X-10 are about. They are their own wrapping row now, each chip sized to its label and 44 px tall, checked at 360 and 390 px. `ToggleButtonGroup` kept one fix from the attempt: it sets `aria-pressed` now, so its other three callers announce which option is on.
...
- **SC-04 · State is lost on every navigation — Medium** (X-14): selected date and search reset; "my matches" links from Home land on the default week. *Effort:* S–M.
  - **Done (W4).** The night and the search text are in the address, and Home's "my match" row names the night and the match. See X-14.
...
- **Done (W6).** Save Round with no signal holds the round instead of failing it. The round shows in the log straight away, a line under the scoreboard reads "Offline — 1 round waiting to sync", and it is sent by itself when the signal returns with nothing pressed. The tapped scores are kept on the phone, so a reload hands them back.
  - **No queue was written.** TanStack Query already parks a mutation fired with no connection — `onMutate` runs, so the optimistic round appears, and the client resumes it on reconnect. All this needed was a `mutationKey` to count what is waiting (`usePausedRoundCount`) and something on screen to say so (`SyncStatusNotice`). A hand-rolled outbox would have been more code for the same result, plus a second thing that can go wrong.
  - **The real work was stopping the panel freezing.** A parked save stays `isPending` for as long as the signal is gone, and `submitRound.isPending` was read in five places in `ActiveGamePanel`. Left alone, a scorer who filed one round offline would have found the score grid disabled, Undo blocked and Save Round spinning until the signal came back — worse than the failure it replaced. Everything now reads `isPending && !isPaused`. For the same reason the offline path fires the save rather than awaiting it: a parked mutation's promise never settles, so awaiting it would strand the scorer on a round they have already filed.
  - **Correction to the finding: `localStorage`, not `sessionStorage`.** The case this exists for is a phone at a venue whose screen locked and whose browser then reclaimed the tab, and session storage does not always survive that. The draft is keyed by game, carries its round number so a round that moved on is not re-offered under the wrong number, and expires after twelve hours so it cannot resurrect last week's league night.
  - **The one honest limit.** The held round itself lives in memory, so closing the tab loses it. The taps are not lost — they come back on the next visit and one press files them again. Making the *send* survive a killed tab needs a durable outbox, which is a separate change.
  - **The "Live updates" pill is untouched.** It reports the realtime channel, not the network, and the two are different axes — the channel can be down on a perfectly good connection. The sync line is a second, separate message.
  - **One trap, found by a review bot after the first push.** Queueing a round advances the round number straight away — that is the point — which ran the "the round moved on under you" path and deleted the very copy the queued save had just kept. Since the queued mutation is memory-only, a tab that died before the signal returned lost the round with nothing left to re-file. That path now only drops the copy when the round moved because *somebody else* recorded it. Two tests missed it because their mocked save never advanced the round; the replacement fails against the old code.
...
- **PO-03 · Current season shows three "No brackets yet for this division" cards and nothing else — Medium.** (`anon/playoffs--d1440.jpg`). Between the regular season and playoffs this is the whole page; no date, no "seeding as of today", no link to standings. *Recommend:* show projected seeds (top N by Power) with "Brackets are created by the admin after week X" and a link to Standings. *Effort:* M.
  - **Done (W9).** An empty division card now reads "Projected seeds" and lists **every** team in that division in seed order with its power score, then "Brackets open after week N.", then a link to `/stats`. The order is `useTeamRankings`' own, which is the same rule `bracket-creator.ts` seeds by, so what the card shows is what creating the bracket would produce.
  - **Active season only, and that is a data limit, not a choice.** Power scores come from `v_team_details`, which carries no `season_id` — it is always the season being played. On any other season the card keeps the old sentence rather than showing numbers that would be quietly wrong. Same fallback while the rankings load, if they fail, and for a division with no teams.
  - **"Week X" had nowhere to come from.** There is no `playoff_start_week` column; `seasons` has only `start_date` and a nullable `end_date`. The week is derived from those two, using the same arithmetic as `useSeasonWeek`, and the sentence drops the number entirely ("Brackets open when the regular season ends.") when `end_date` is empty. No migration.
  - **The seeds are passed as a slot, not fetched by the list.** `useTeamRankings` fetches teams, matches and the snapshot baseline unconditionally — its arguments do not gate the requests. `BracketList` hands `DivisionBracketsCard` an element it renders *only* in its empty branch, so a division that already has a bracket costs nothing, and the views pass no season while a bracket is open. This also keeps `DivisionBracketsCard` free of providers, which is what its own tests rely on.
...
- **A-01 · Admin sections have no URLs: Back leaves the console, links can't be shared, and the mobile menu doesn't follow programmatic switches — High.** *Observed:* after approving a submission in Pending, pressing Back lands on `/` (J9); after League Night's "Live corrections" quick action the phone menu still shows the *Operations* group open and nothing highlighted (`admin/ln-quick-action-live-corrections--m390.jpg`, `navState.groupsOpen = []`). *Code:* `AdminSidebar.tsx` keeps the tab in `sessionStorage`; `AdminMobileNav.tsx:128-133` computes open groups once. *Recommend:* `/admin/:section` routes (or `?tab=`) with `useSearchParams`; derive the open group from the active tab. *Effort:* M. *Accept:* Back returns to the previous section; `/admin/pending-matches` opens Pending; the active item is highlighted after a quick action.
  - **Done (W1).** Every section is an address: `/admin/scores`, `/admin/pending-matches`, and so on. Back steps between sections, a section can be shared or bookmarked, and the phone menu follows the section in the address rather than computing its open group once. Path params rather than `?tab=`: a section is a place, and query params are wanted for filter state elsewhere (W4).
  - **One registry.** The 21-item menu was duplicated in the sidebar and the phone nav. Both read `src/components/admin/dashboard/adminSections.ts` now, which also ended the "Hero" / "Hero Cards" label drift noted in A-15.
  - **Two old addresses fell out of it.** `/admin/notifications` needed a redirect only because sections had no addresses; it is the Notifications section's own address now. `/timeslots` is a plain redirect, and `LegacyAdminRedirect` is deleted.
  - **A change worth knowing.** A section switch is a real navigation, so it records a pageview, is announced, and moves keyboard focus into the section — like every other page change in the app. `routeName.ts` needed an `/admin/` entry or the announcer read out "Page Not Found" on each switch.
...
- **A-05 · Approving a request changes only a status word — High (inferred, code).** `useTeamRequests.ts:99-104` writes `status/admin_notes/processed_by`; a TIME_CHANGE approval moves nothing and the dialog offers no route to Timeslots (`admin/requests-approve-dialog--m390.jpg`: "Approve Request" with notes only; no success toast captured after confirm). *Recommend:* after approve, toast "Approved — now move {team} to {requested_timeslot} in Timeslots" with an action calling `switchAdminTab('timeslots')`; wrap `mutateAsync` in try/catch so a failure doesn't leave the dialog open with a live button (`RequestsTab.tsx:44-56`). *Effort:* S. *Accept:* the admin can reach the matching Timeslots date from the approval toast.
  - **Half done (Q8).** The try/catch landed, and the toast named the work left with an **Open Timeslots** action. It carried no date, though, so the acceptance line above was not met: the button opened the section on whatever night it happened to default to.
  - **Done (L4).** The toast carries the night, the team and the block, and Timeslots opens on them with a card that makes the change in one press. All three request types now carry it, not just `TIME_CHANGE`. Approving still writes only a status — that is deliberate, and the reasoning, the write order and the refusals are in the notes under §5.3's table.
...
- **A-07 · Unsaved work is lost on any section switch — High (inferred, code; partially observed).** No route blocker exists; `AutoScheduleTab.tsx:75-88` is the only `beforeunload` and it ignores generated-but-unsaved schedules (`index.ts:185-189`); Mass Score Entry keeps local edits (`useScoreEntryData.ts:68`) but switching sections discards them silently; the hero-card form replaces the list with no guard (`HeroCardsTab.tsx:33-35`); `ScoreSubmissionModal` resets on backdrop tap. *Recommend:* one `useUnsavedChangesGuard(dirty)` hook (beforeunload + `window.confirm` in `AdminSidebar.handleTabChange`) wired to Mass Score Entry, Auto Schedule, Hero form, Edit Round and Blind Draw settings. *Effort:* M. *Accept:* switching sections with unsaved scores asks first.
  ...
  - **Auto Schedule's dirty test was too narrow, and is wider now.** `hasUnsavedEdits` was only true in edit mode. A successful save clears the persisted copy but leaves `generatedMatches` in state, so "a schedule exists" could not stand in for "unsaved": the save itself is tracked instead (`hasUnsavedWork`), and generating or applying marks the work unsaved again. **A save made in edit mode now settles `hasUnsavedEdits` too**: it re-baselines `generatedMatches` to the set it just wrote, so the diff that drives the flag closes. Without that the alert and the leave guards survived a successful save, and Save stayed enabled — a second press wrote the night twice. Edit mode deliberately stays on: Save and Reset are gated on `isEditMode && hasUnsavedEdits`, so the closed diff already retires them, whereas leaving edit mode would render the preview off `generatedPairings` — which editing never touches — and offer a second Save that writes the pre-edit night over the edited one.
...
- **A-11 · Live Corrections opens on "All seasons" with no date or team filter; on a phone the detail panel is below the whole list — Medium.** *Observed:* selecting a match scrolls nowhere (`scrollY` unchanged, `admin/lc-match-selected--m390.jpg`); "Clear selection" is a ghost button at the bottom right. *Recommend:* default to the active season + tonight; `scrollIntoView` on selection; a "Back to list" affordance. *Effort:* S.
  - **Done (W10).** The section opens on the active season and on the most recent night that has live-scored matches, with a Night picker beside the Season one. Both widen in one tap ("All seasons", "All nights"). On a phone, choosing a match scrolls the panel into view and a "← Back to list" button at the top of the panel scrolls back; on a desktop neither happens and the same button reads "Clear selection". It only exists while a match is selected, rather than sitting disabled in the bottom-right corner.
  - **"Tonight" is read off the data, not the calendar.** A hard "today" filter would show an empty list on any night the league does not play, and the empty copy would then lie. The night options are the nights actually present in the loaded list, and the default is `pickDefaultEntryDate` — the same A-03/Q6 helper Mass Score Entry uses — so the default can never land on a night with nothing on it.
  - **The empty line now names what emptied it.** "No live-scored matches in this season." / "No live-scored matches on that night." / "No live-scored matches yet." under All seasons. That closes the LIVEFIX-17 half of this finding.
  - **Nights are read in league time**, so a match stored at midnight UTC is filed under the evening before — the night it was played — and the picker and the cards agree wherever the browser is. The card's date line was local before, which made it timezone-dependent.
  - **Filtering stayed in the component.** The query is still season-scoped server-side; the night narrows the loaded list. No change to `useAdminLiveScoredMatches`, its query key, or `AdminCorrectionsService`. Not in the URL either — that slot is still free.
- **A-12 · Seasons: archived seasons remain editable, only the active season can be archived, end date is not validated — Medium (inferred; edit buttons observed on every archived card in `admin/tab-seasons--d1440.jpg`).** `SeasonsList.tsx:130-140`, `SeasonManagementTab.tsx:90`, `SeasonForm.tsx:22-26`. *Effort:* S.
  - **Fixed (Q30), and it was not an S.** Exposing Archive on an inactive season needed a database change first: `archive_season` and `partial_archive_season` both ended with an unconditional `UPDATE public.teams SET wins = 0, losses = 0, game_wins = 0, game_losses = 0 WHERE id IS NOT NULL` — the live league-wide standings, which carry no season id. That is right when archiving the season being played and destructive otherwise, and it was safe only because the UI restricted Archive to the active season. Migration `20260909120000_scope_archive_counter_reset_to_active_season.sql` guards both resets on `is_active`, and stops `archive_season` stamping `CURRENT_DATE` over a historical end date. Proven by a new case in `supabase/tests/season_rollover_workflow.sql`, which fails against the old function definition. **The migration must be applied by hand before the UI ships — see `docs/OPERATIONS.md` §6.**
...
- **A-14 · Participation table has no overflow wrapper, exports a UUID-named CSV without quote escaping — Low.** `SeasonParticipationTab.tsx:211,94,100` (axe `scrollable-region-focusable` on 390). *Effort:* S.
  - **Overflow half done (L3).** The participation table is the first consumer of `ResponsiveTable`, so below `md` it is a stack of cards rather than a table in a sideways-scrolling box. There is no scrollable region left to focus, so the axe rule cannot fire. **The CSV half is untouched:** it is still named after the season UUID and still wraps every cell in quotes without escaping the quotes inside them.
...
| W7 ✅ | One contact form with a unified "What is this about?" select; compact home card (two mailboxes kept — see H-02 notes) | H-02 |
...
- **Done.** `TableHead` supplies `scope="col"`, which fixed eleven files in one line; six more hand-write `<th>` and got it directly. That is 15 files, not the 9 this row counted — the count only looked at `src/components/admin/`, and head-to-head, compact standings, the completed-match review, the team season stats, timeslots and the playoff division table had the same gap on public pages. `table.tsx` got its first test, and it fails against the old code.
- **`ResponsiveTable`** (`src/components/ui/responsive-table.tsx`) renders a real table from 768px and a labelled list of cards below it, from one `columns` array. Adopted by season participation, opponent history, hero cards and the admin teams pane. `src/docs/TABLE_PATTERNS.md` is the guide, and carries the list of tables that deliberately do not use it.
- **Two corrections to this row.** `code §4d` is not in this repository — it points at a code review that was never committed, so the only in-repo justification for L3 was this row plus ST-04. And **ST-04 was already closed by Q14**: it is a one-line pointer to X-10, whose tap-target and font-size fixes shipped there. The live reasons for doing L3 were the missing `scope` and the four competing phone patterns, not the sizing.
- **"Three ranking components are duplicated" did not survive contact with the code.** The rankings and career phone views carry a league leaderboard carousel, team badges, a Compact/Detailed toggle persisted to `localStorage`, a Division/All toggle and a deliberately reduced 4-field sort row against the desktop's 8 and 10 sortable columns. §3.5 of this report lists that phone view under **Works well**. Head-to-head's card is colour-coded down its left edge by rivalry type and packs six figures into three dense rows. Folding any of them into a generic card mode would have deleted a real design and called it a refactor. They keep their views and are named as exceptions.
- **What was genuinely duplicated, and is now gone.** `TeamTableDesktop` and `TeamListMobile` had a character-for-character identical `TeamAvatar` and a division `<Select>` differing only by width — one `TeamsTable` replaces both. `rivalryBadgeConfig` was copied verbatim between `HeadToHeadRecords.tsx` and `H2HMobileCard.tsx`; it lives in `rivalryUtils.ts` now.
- **Head-to-head's desktop table was never the shared primitive at all.** It was a raw `<table>` with a local `SortButton` that showed the same up-and-down icon on every column — so neither a sighted reader nor a screen reader could tell which column was sorted. It is on `Table` + `SortableColumnHeader` now and announces `aria-sort`. Its opponent name was a `role="button"` div with a hand-written `onKeyDown`; it is a real `<Button>`.
- **`table.tsx` no longer needs a router.** It called `useSeasonalTheme()`, which calls `useLocation()`, while reading only `isWinterTheme` — so every table in the app required a Router and re-rendered on every route change. This had to be fixed first: the test for the `scope` default renders a whole `<Table>`, and `HeadToHeadRecords.test.tsx` has no Router wrapper.
- **Not done: DivisionsTab.** It was the planned first consumer, on the grounds that it is the best existing card-mode in the repo. Its rows share inline-edit state across their cells — one Edit button turns three cells into inputs — and a `cell: (row) => ReactNode` function cannot hold React state. Hoisting that state would have been a behaviour change riding along in a design commit. It keeps its two-tree layout and is recorded as an exception.
- **Not done: `ComparisonTable`, `PreviewRankingTable`, `TimeslotList`, `CompactStandings`.** Two header rows and a nested detail table; existing column-dropping that works; three short columns that already read fine on a phone; and a `VirtualizedList` of cards that cannot live inside a `<table>`. All four got `scope` for free and are listed with their reasons.

**Notes on L4.**

- **Done, as the "or opens Timeslots prefilled" half — deliberately, not as a compromise.** Approving still writes only a status. The approval toast now carries the night, the team and the block into `/admin/timeslots?date=&team=&slot=`, and a card there states what changing it would do and does it in one press. The alternative — approving writes the schedule — was rejected because it moves a team from a screen that cannot show the night: it cannot see that the target block is full, and it cannot catch a time the player typed wrong. Six presses became two, which is what the row asked for.
- **All three request types carry it, not just `TIME_CHANGE`.** A bye request and an **emergency cancellation** both point at `BYE`: either way the team is not playing that night. A-05 and Q8 covered time changes only, so those two approvals used to end in a plain "Request Approved" with no route anywhere.
- **The write order is the correctness story, and it is the opposite of the obvious one.** `team_timeslots` has no update path, so every move is a delete and an insert. Clearing first and booking second leaves the team with **no booking at all** when the booking fails — a hole no screen shows, found on league night. `moveTeamBooking` (`src/hooks/useTimeslotMutation.ts`) books first and clears second, so the worst case is a team booked twice, which sits in the night's own list; that case gets its own message naming the repair rather than reporting the move as a failure.
- **That order needed a new delete.** `BackToBackTimeslotService.deleteTimeslot` deletes by *rule* — team, night, `is_back_to_back` — so it would have taken the rows just written. `TimeslotBatchService.deleteTimeslotsByIds` takes exactly the ids read before the booking. An empty list sends no query at all, rather than a delete with no filter.
- **A double header is refused, not guessed at.** A team holding two blocks that night has two bookings a time change could mean, and the request never says which. The card names both and asks the admin to remove the one they mean. A **bye** on the same team is still one press, because a bye means no games at all — and the card names every game it removes. Guessing here would delete a game.
- **`requested_timeslot` is free text and always has been.** The form is a plain box with "e.g., 7:00 PM" as a hint and no constraint in the database. `parseRequestedBlock` (`src/utils/timeslotMove.ts`) reads it under one rule: **it may answer "I do not know", it may never be wrong.** Exact shapes only — `7:00 PM`, `7pm`, `7`, `19:00` — and `null` for everything else, including `9:30 PM`, which is a legal stored time that starts no block. On `null` the shortcut still works, smaller: the night and team are carried, no block is chosen, and the card quotes what the team actually typed.
- **The root cause is fixed too: the request form asks for a block.** Both time fields on the home request card were plain text boxes with "e.g., 7:00 PM" as the only guidance. They are now pickers of the league's nine blocks, labelled by both times they cover ("6:30 + 7:00 PM") and storing the block's first time — the exact value a booking takes. A time change cannot be sent without one, because a time change that names no time is nothing anyone can act on. `parseRequestedBlock` stays as the reader of every row written before this, which is why it was built to say "I do not know" rather than to guess.
- **Two things a review caught, both about acting on rows that are not this night's.** The list keeps the previous night's rows on screen while a newly chosen night loads, so the card had to be held back until the rows really are the chosen night's — a move clears **by row id**, and planning against stale rows would have deleted a booking on the night before. And the address had to be re-read rather than read once: a second approval arriving while Timeslots was already open changed the address and put nothing on screen. Reading it every render is safe because dismissing removes the instruction from the address rather than ignoring it.
- **The address is an instruction, not view state.** The three parameters are read once on arrival and taken back out once the card is done with or dismissed. The Timeslots date is still **not** URL state — `manage-timeslots.md` documents its absence, and putting it there would change what a reload and a Back both do. A separate change if it is wanted.
- **A-05's acceptance line is met for the first time.** It asked that "the admin can reach the matching Timeslots date from the approval toast". Q8's toast carried no date at all, so it met the first half of the finding (a route to Timeslots, and the try/catch) and not this one.
- **One limit worth knowing, stated on the card.** A move changes when a team is *expected*. It does not change a match already created for that night: `matches` carries its own date and no timeslot. Teams are still not told their time changed, which was already true of every timeslot change.
