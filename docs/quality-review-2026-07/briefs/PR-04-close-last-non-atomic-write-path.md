...
1. In `MassScoreEntryTool.handleDeleteConfirm`, replace the three-call sequence with `await deleteMatchWithStatsReversal(deleteMatchId);` mirroring `useMatchDelete.ts:40`; keep the toasts and `invalidateAllDataQueries`. Remove the now-unused imports and the `['mass-score-entry']` dead invalidation key (nothing subscribes to it).
2. Delete `EditScoresSection.tsx` + `useUncompletedMatches.ts` + tests (zero importers — verified by grep).
...
4. `createMatch`: strip `iscompleted/winner_id/loser_id/team1_score/team2_score` from `MatchInsertInput` (mirror the `MatchNonResultUpdate` type-guard pattern at MatchWriteService.ts:25-30). If a future "create completed match" flow is wanted, it must insert-then-call `resubmit_match_result`.
