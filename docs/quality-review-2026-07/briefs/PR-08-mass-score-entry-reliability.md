...
1. **Submission failures never reach the error UI that already exists.** The display plumbing is all in place — `ErrorAlert` is mounted (`MassScoreEntryTool.tsx:151`) and `MatchesTable` receives `failedMatches`/`errorMessages` (`:158-159`) — but `useErrorHandling`'s `addError` (`useErrorHandling.ts:21-24`) is **never called from the submit path**, so the state stays empty forever and the banner/row badges never have anything to show. The fix is wiring `handleSubmitAll`'s per-item catch into `addError` (and successes into `clearErrors`), not building new UI. A league admin whose batch half-fails currently sees only a generic toast and has no idea *which* matches to re-submit.
...
1. Wire failures into the existing error state: in `handleSubmitAll`'s per-item catch, call `addError(matchId, message)`; on success, `clearErrors(matchId)`. The already-mounted `ErrorAlert` + `MatchesTable` badges then light up. Extend `ErrorAlert` with the batch summary ("7 saved, 2 failed") and a "Retry failed" button (re-submits only the failures); verify the per-row dismiss (`clearErrors(matchId)`) is wired to the row's X button.
2. Fix the count: compute the button label from the **exact predicate the submit loop uses** — `match.isEdited && match.isValid && match.iscompleted` (`useScoreEntryData.ts:85-86`); extract it into one shared function so label and loop can never diverge. For edited-but-not-submittable rows, show an inline hint ("invalid — fix scores" / "marked incomplete — won't submit") and decide the un-complete story: either support it via the existing reopen RPC, or disable un-completing in this tool with a pointer to Live Corrections. (Recommend the latter for scope control; note it in the UI.)
3. Replace `Promise.all` with **strictly serial submission (concurrency 1)** — a plain `for … await` loop with per-item error capture. Concurrency 2 does not remove the deadlock risk: `resubmit_match_result` updates the winner row then the loser row, so two concurrent matches touching the same teams in reversed order can still wait on each other. Serial is the simple correct choice here (the RPCs serialize on `team_season_stats` row locks anyway, so wall-clock cost is near zero); if parallelism is ever wanted, it requires an explicit DB-side lock-ordering (always lock lower team-uuid first) or retry-on-deadlock strategy — out of scope.
