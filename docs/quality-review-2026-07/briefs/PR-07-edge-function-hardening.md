...
4. `rateLimit.ts`: key on the **last** XFF hop (the hop appended by Supabase's own edge) or `x-real-ip` when present; on RPC error **fail closed** for unauthenticated form endpoints (return 429 with retry-after) — a rare false-positive beats an open spam channel. Salt the IP hash with an env secret (`IP_HASH_SALT`).
