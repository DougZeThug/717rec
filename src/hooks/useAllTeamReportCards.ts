...
export function useAllTeamReportCards(mode: ReportCardMode) {
  ...
  const leaderboard = useMemo((): LeaderboardEntry[] => {
    if (mode === 'career') {
      ...
      return careerRankings.map((team) => {
        const overallGrade = calculateGrade(calculatePercentile(team.careerPowerScore, allPowerScores, true).percentile);
        const consistencyGrade = calculateGrade(calculatePercentile(team.careerWinPercentage, allWinPcts, true).percentile);
        const gamesGrade = calculateGrade(calculatePercentile(team.careerGameWinPercentage, allGameWinPcts, true).percentile);
        const offenseGrade = calculateGrade(calculatePercentile(team.careerSweepRate, allSweepRates, true).percentile);
        const clutchPercentile = team.careerClutchGame3s > 0 ? Math.round(team.careerClutchWinPct) : 50;
        const clutchGrade = calculateGrade(clutchPercentile);
        const scheduleGrade = calculateGrade(calculatePercentile(team.careerSos, allSos, true).percentile);
    ...
    return rankings.map((team, idx) => {
      const overallGrade = calculateGrade(calculatePercentile(team.powerScore, allPowerScores, true).percentile);
      const consistencyGrade = calculateGrade(calculatePercentile(team.winPercentage, allWinPcts, true).percentile);
      const gamesGrade = calculateGrade(calculatePercentile(team.gameWinPercentage, allGameWinPcts, true).percentile);
      const offenseGrade = calculateGrade(calculatePercentile(allSweepRates[idx], allSweepRates, true).percentile);
      const clutchGrade = calculateGrade(50); // Neutral when no per-team match data
      const scheduleGrade = calculateGrade(calculatePercentile(team.sos, allSos, true).percentile);
