...
export const useEditableMatches = (
  ...
  const swapTeams = useCallback(
    (matchId: string, setEditableMatches: (matches: AutoScheduleMatch[]) => void) => {
      setEditableMatches(
        currentEditableMatches.map((match) => {
          if (match.id === matchId) {
            return {
  ...
  const removeMatch = useCallback(
    (matchId: string, setEditableMatches: (matches: AutoScheduleMatch[]) => void) => {
      setEditableMatches(currentEditableMatches.filter((match) => match.id !== matchId));
    },
    [currentEditableMatches]
  );
  ...
  const hasUnsavedEdits = useCallback(
    (generatedMatches: AutoScheduleMatch[]) => {
      ...
      return currentEditableMatches.some((editedMatch, index) => {
        ...
        return (
          editedMatch.team1Id !== originalMatch.team1Id ||
