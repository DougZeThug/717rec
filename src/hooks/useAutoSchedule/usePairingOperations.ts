...
import {
  ...
  logQualityAnalysis,
} from '@/utils/autoSchedule/qualityAnalysis';
import {
...
export const usePairingOperations = (
  ...
  const handleGenerateClick = useCallback(
    async (
      ...
      const totalTeams = Object.values(timeBlockTeams).reduce(
      ...
      try {
        ...
        const result = await generateMatchPairings(
          selectedDate,
          timeBlockTeams,
          config,
          teamBlockMap
        ...
        if (result) {
          ...
          toast({
            ...
            description:
              'Failed to generate schedule. Please check the console for details and try again.',
            variant: 'destructive',
          });
        }
      } catch (error) {
    ...
    (
      ...
      try {
        ...
        const averageCompatibilityScore =
          pairingCount > 0 ? totalCompatibilityScore / pairingCount : 0;
        const qualityRating =
          averageCompatibilityScore >= 7
            ? 'Excellent'
            : averageCompatibilityScore >= 5
              ? 'Good'
              : averageCompatibilityScore >= 3
                ? 'Fair'
                : 'Poor';
        ...
        scheduleLog(
          `Applied schedule: ${matches.length} matches, ${rematchCount} rematches, quality: ${qualityRating}`
        );
