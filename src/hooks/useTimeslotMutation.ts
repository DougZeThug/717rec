...
export const useTimeslotMutation = () => {
  ...
  const addTimeslot = async (date: Date, teamId: string, timeslot: string): Promise<TeamTimeslot | null> => {
    // Validate input data
  ...
  const batchAssignTimeslots = async (date: Date, teamIds: string[], timeslot: string): Promise<TeamTimeslot[] | null> => {
    // Validate input data
