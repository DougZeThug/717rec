  ...
  const actionMutation = useMutation({
    ...
    onError: (error, { status }, context) => {
      if (context?.removedSubmission) {
        ...
        queryClient.setQueryData<ScoreSubmission[]>(scoreSubmissionKeys.all, (curr = []) =>
          curr.some((sub) => sub.id === removedSubmission.id) ? curr : [...curr, removedSubmission]
        );
      }
    ...
    onSettled: () =>
      queryClient.invalidateQueries({ queryKey: scoreSubmissionKeys.all, refetchType: 'active' }),
  });
