...
export class HeadToHeadService {
  ...
  static async getTeamHeadToHead(teamId: string): Promise<HeadToHeadRecord[]> {
    ...
    try {
      ...
      matchLog(
        `Head-to-head complete: ${result.length} records in ${(endTime - startTime).toFixed(2)}ms`
      );
  ...
  static async getOpponentHistory(
    ...
    try {
      ...
      const endTime = performance.now();
      matchLog(
        `Opponent history complete: ${matchHistory?.length || 0} matches in ${(endTime - startTime).toFixed(2)}ms`
      );
