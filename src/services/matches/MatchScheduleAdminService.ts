 ...
 *
 * Paginated (via fetchAllPages) like fetchMatchesForAdmin above: PostgREST caps
 * a response at 1,000 rows and says nothing about it, so a season past that size
 * used to hand the /schedule page a short schedule with no error to show for it.
 * Ordered by date then id — date alone is not a total order, and range
 * pagination needs a unique sort or it can skip or repeat rows across pages.
 *
 * @throws {DatabaseError} When database operations fail
 */
export const fetchScheduleMatches = async (): Promise<ScheduleMatchWithTeams[]> => {
  ...
  return fetchAllPages<ScheduleMatchWithTeams>((from, to) => {
    return supabase
      .from('matches')
      .select(
        `
      ...
      .order('id', { ascending: true })
      .range(from, to);
  }, 'Failed to fetch schedule matches');
};
