...
describe('deleteBracket', () => {
  ...
  it('throws DatabaseError on error', async () => {
    mockFrom.mockReturnValue({ delete: () => ({ eq: () => Promise.resolve({ error: pgError() }) }) });
...
describe('updatePlayoffMatchResult', () => {
  ...
  it('resolves on success', async () => {
    ...
    await expect(updatePlayoffMatchResult('m-1', { winner_id: 't1', loser_id: 't2', team1_score: 2, team2_score: 1, status: 'complete' })).resolves.toBeUndefined();
    expect(mockFrom).toHaveBeenCalledWith('playoff_matches');
  });

  it('throws DatabaseError on error', async () => {
    mockFrom.mockReturnValue({ update: () => ({ eq: () => Promise.resolve({ error: pgError() }) }) });
    await expect(updatePlayoffMatchResult('m-1', { winner_id: 't1', loser_id: 't2', team1_score: 2, team2_score: 1, status: 'complete' })).rejects.toThrow(DatabaseError);
  });
});
...
describe('upsertPlayoffGame', () => {
  ...
  it('resolves on success', async () => {
    ...
    await expect(upsertPlayoffGame({ id: 'g-1', match_id: 'm-1', game_number: 1, team1_score: 2, team2_score: 1, winner_id: 't1' })).resolves.toBeUndefined();
  });

  it('throws DatabaseError on error', async () => {
    mockFrom.mockReturnValue({ upsert: () => Promise.resolve({ error: pgError() }) });
    await expect(upsertPlayoffGame({ id: 'g-1', match_id: 'm-1', game_number: 1, team1_score: 2, team2_score: 1, winner_id: null })).rejects.toThrow(DatabaseError);
  });
});
...
describe('updatePlayoffMatchScores', () => {
  ...
  it('resolves on success', async () => {
    mockFrom.mockReturnValue({ update: () => ({ eq: () => Promise.resolve({ error: null }) }) });
    await expect(updatePlayoffMatchScores('m-1', { team1_score: 2, team2_score: 1, winner_id: 't1', loser_id: 't2', status: 'complete', updated_at: '2026-04-17' })).resolves.toBeUndefined();
  });

  it('throws DatabaseError on error', async () => {
    mockFrom.mockReturnValue({ update: () => ({ eq: () => Promise.resolve({ error: pgError() }) }) });
    await expect(updatePlayoffMatchScores('m-1', { team1_score: 2, team2_score: 1, winner_id: 't1', loser_id: 't2', status: 'complete', updated_at: '2026-04-17' })).rejects.toThrow(DatabaseError);
  });
});
...
describe('insertPlayoffGames', () => {
  ...
  it('resolves on success', async () => {
    ...
    await expect(insertPlayoffGames([{ match_id: 'm-1', game_number: 1, team1_score: 2, team2_score: 1, winner_id: 't1' }])).resolves.toBeUndefined();
  });

  it('throws DatabaseError on error', async () => {
    mockFrom.mockReturnValue({ insert: () => Promise.resolve({ error: pgError() }) });
    await expect(insertPlayoffGames([{ match_id: 'm-1', game_number: 1, team1_score: 2, team2_score: 1, winner_id: null }])).rejects.toThrow(DatabaseError);
  });
});
...
describe('updateTeamSeed', () => {
  ...
  it('returns updated seed data on success', async () => {
    ...
    mockFrom.mockReturnValue({
      update: () => ({ eq: () => ({ select: () => ({ single: () => Promise.resolve({ data: row, error: null }) }) }) }),
    });
...
describe('batchUpdateTeamSeeds', () => {
  ...
  it('returns rpc data on success', async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith('batch_update_team_seeds', { p_updates: [{ team_id: 't-1', seed: '1' }] });
