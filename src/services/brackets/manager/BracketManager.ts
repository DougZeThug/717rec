...
interface BracketsManagerAdapter {
  insert(data: any[]): Promise<boolean>; // Changed to boolean to match expected type
  ...
  insertInto(table: string, data: any): Promise<boolean>; // Changed to boolean to match expected type
...
const adapterWithLegacySupport: BracketsManagerAdapter = {
  ...
  insert: async (data: any[]): Promise<boolean> => {
    // Keep the implementation, which already returns boolean
    return bracketsAdapter.insert(data);
  },
  ...
  insertInto: async (table: string, data: any): Promise<boolean> => {
    // Keep the implementation, which already returns boolean
    return bracketsAdapter.insertIntoTable(table, data);
  },
