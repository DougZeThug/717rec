 ...
 * Only protects COMPLETED matches (status >= 4) — in-progress matches allow null
 * writes so brackets-manager can propagate BYE advancement correctly.
 */
...
export class SupabaseSqlStorage implements CrudInterface {
  ...
  private transformMatchFromDb(data: DbMatch): BmMatch {
    ...
    if ('opponent1_id' in data || 'opponent1_score' in data || 'opponent1_result' in data) {
      ...
      // When ID, score, and result are ALL null → true BYE (return null so hasBye() works)
      if (opponentId == null && data.opponent1_score == null && data.opponent1_result == null) {
    ...
    if ('opponent2_id' in data || 'opponent2_score' in data || 'opponent2_result' in data) {
      ...
      // When ID, score, and result are ALL null → true BYE (return null so hasBye() works)
      if (opponentId == null && data.opponent2_score == null && data.opponent2_result == null) {
