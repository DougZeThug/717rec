...
export function computeBracketState(bracket: PlayoffBracket): BracketState {
  ...
  if (bracket.format === BRACKET_FORMATS.DOUBLE) {
    ...
    if (championshipMatch && championshipMatch.winnerId) {
      ...
      const winnerFromWinnersBracket = bracket.matches
        .find(m => m.matchType === 'winners' && m.winnerId && !m.nextWinMatchId);
        
      if (winnerFromWinnersBracket && championshipMatch.winnerId === winnerFromWinnersBracket.winnerId) {
