...
export const useOptimisticTeamMutations = () => {
  ...
  const createOptimisticUpdate = useCallback((teamId: string, newSeed: number | null): OptimisticUpdate => {
  ...
  const rollbackUpdates = useCallback((teamIds?: string[]) => {
  ...
  const optimisticBatchUpdate = useMutation({
    ...
    onSuccess: (result, variables) => {
      ...
      const successfulTeamIds = result.results
        .filter(r => r.success)
        .map(r => r.team_id);
  ...
  const manualRollback = useCallback((teamIds?: string[]) => {
