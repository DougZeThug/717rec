...
export const useBracketFormData = (
  ...
  const shouldUseProvidedTeams = providedTeams && Array.isArray(providedTeams) && providedTeams.length > 0;
  ...
  const isDataReady = !isLoadingTeams && teamsToProcess && Array.isArray(teamsToProcess) && divisions && Array.isArray(divisions);
  ...
  const { processedTeams, processingError } = React.useMemo(() => {
    ...
    try {
      ...
      const processed: ProcessedTeam[] = teamsWithMixedSeeds.map((team) => ({
        ...
        divisionName: (team as any).divisionName || (team as any).divisionname || 'Unknown Division',
