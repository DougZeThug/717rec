 ...
 */
type BracketFormTeam = PlayoffTeam & {
...
const getTeamDivisionName = (team: BracketFormTeam): string =>
  team.divisionName ?? team.divisionname ?? 'Unknown Division';

export const useBracketFormData = (
  divisions: Division[] = [],
  providedTeams?: BracketFormTeam[],
  divisionId?: string
  ...
  const teamsToProcess: BracketFormTeam[] = shouldUseProvidedTeams
    ? providedTeams
    : (fetchedTeams ?? []);
