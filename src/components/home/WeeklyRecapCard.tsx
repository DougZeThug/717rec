...
const UpsetsSection: React.FC<{ upsets: WeeklyUpset[]; winter: boolean }> = ({ upsets, winter }) => {
  const hasUpsets = upsets.length > 0;
  if (!hasUpsets) return null;
...
const StreaksSection: React.FC<{ streaks: TeamStreakInfo[]; winter: boolean }> = ({ streaks, winter }) => {
...
const MoversSection: React.FC<{ risers: WeeklyPowerScoreTrend[]; faller?: WeeklyPowerScoreTrend; winter: boolean }> = ({ risers, faller, winter }) => {
  const hasMovers = risers.length > 0 || !!faller;
...
const MobileUpsetRow: React.FC<UpsetRowProps> = ({ upset, winter }) => (
  <div className="flex items-center justify-between gap-1.5">
    <div className="flex flex-col gap-1 min-w-0">
      <Link to={`/teams/${toTeamSlug(upset.winnerName)}`} className="flex items-center gap-1.5 group min-w-0">
        <TeamLogo imageUrl={upset.winnerLogoUrl} teamName={upset.winnerName} size="xs" />
        <span className={cn('text-xs font-medium transition-colors leading-tight', winter ? 'text-cyan-50 group-hover:text-cyan-300' : 'group-hover:text-violet-600 dark:group-hover:text-violet-400')}>
          {upset.winnerName}
        </span>
      </Link>
      <Link to={`/teams/${toTeamSlug(upset.loserName)}`} className="flex items-center gap-1.5 group min-w-0">
        <TeamLogo imageUrl={upset.loserLogoUrl} teamName={upset.loserName} size="xs" />
        <span className={cn('text-xs transition-colors leading-tight', winter ? 'text-cyan-100/70 group-hover:text-cyan-300' : 'text-muted-foreground group-hover:text-violet-600 dark:group-hover:text-violet-400')}>
          {upset.loserName}
        </span>
      </Link>
    </div>
...
const MobileStreakRow: React.FC<StreakRowProps> = ({ team, winter }) => (
  <Link to={`/teams/${toTeamSlug(team.teamName)}`} className="flex items-center gap-1.5 group">
    ...
    <span className={cn('text-xs font-medium transition-colors flex-1 min-w-0 leading-tight', winter ? 'text-cyan-50 group-hover:text-cyan-300' : 'group-hover:text-violet-600 dark:group-hover:text-violet-400')}>
      {team.teamName}
    </span>
    <span className="shrink-0 text-[10px] font-bold tabular-nums bg-orange-500/20 text-orange-600 dark:text-orange-400 px-1.5 py-0.5 rounded">{team.streak}</span>
  </Link>
);
...
const StreakRow: React.FC<StreakRowProps> = ({ team, winter }) => (
  <Link to={`/teams/${toTeamSlug(team.teamName)}`} className="flex items-center gap-2 group">
    ...
    <div className="flex-1 min-w-0">
      <span className={cn(typeScale.body, 'font-medium truncate transition-colors block', winter ? 'text-cyan-50 group-hover:text-cyan-300' : 'group-hover:text-violet-600 dark:group-hover:text-violet-400')}>
        {team.teamName}
      </span>
      <span className={cn(typeScale.caption, 'text-muted-foreground truncate block')}>{team.division}</span>
    </div>
  ...
  return (
    <Link to={`/teams/${toTeamSlug(trend.teamName)}`} className="flex items-center gap-2 group">
      ...
      <div className="flex items-center gap-1 shrink-0">
        {isUp ? <TrendingUp size={14} className="text-emerald-500" /> : <TrendingDown size={14} className="text-red-500" />}
        <span className={cn('text-sm font-semibold tabular-nums', isUp ? 'text-emerald-500' : 'text-red-500')}>
          {isUp ? '+' : ''}
