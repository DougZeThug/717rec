...
const TeamsHeader: React.FC<TeamsHeaderProps> = ({ title, description, children }) => {
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1 sm:gap-3 pb-2 sm:pb-4 border-b border-border">
      ...
      {description && (
        <p className="hidden sm:block text-sm md:text-base text-muted-foreground">{description}</p>
      )}
    </div>
  );
};
