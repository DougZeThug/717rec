...
vi.mock('@/components/ui/CollapsibleSection', () => ({
  CollapsibleSection: ({ children, title }: any) => <section><h2>{title}</h2>{children}</section>,
}));

const mockReportCard = vi.hoisted(() => vi.fn());
const mockAdvanced = vi.hoisted(() => vi.fn());
vi.mock('@/hooks/useTeamReportCard', () => ({ useTeamReportCard: (...args: any[]) => mockReportCard(...args) }));
vi.mock('@/hooks/useTeamSeasonBreakdown', () => ({ useTeamSeasonBreakdown: (...args: any[]) => mockAdvanced(...args) }));
...
describe('Computed stats rendering', () => {
  it('renders zero-match values correctly', () => {
    render(<TeamStats wins={0} losses={0} gameWins={0} gameLosses={0} winPercentage="0.0" gameWinPercentage="0.0" sos={0} powerScore={0} />);
    expect(screen.getAllByText('0-0').length).toBeGreaterThan(0);
    expect(screen.getAllByText('0.0%').length).toBeGreaterThan(0);
  });

  it('report card missing stats fallback', () => {
    mockReportCard.mockReturnValue({ grades: null, isLoading: false });
    render(<TeamReportCard teamId="t1" standalone />);
    expect(screen.getByText(/not enough data/i)).toBeInTheDocument();
  });

  it('advanced section missing seasons fallback', () => {
    mockAdvanced.mockReturnValue({ advancedStats: { seasons: [] }, isLoading: false });
