...
describe('TeamPlayerStatsSection', () => {
  it('renders one card per player with PPR and DPR computed from raw counters', async () => {
    ...
    // The section is collapsed by default — open it to see the cards.
    ...
    expect(screen.getByText('+2.75')).toBeInTheDocument(); // (145 - 90) / 20, signed
    expect(screen.getByText('20 rounds')).toBeInTheDocument();
    expect(screen.getByText('5–2')).toBeInTheDocument();
  });

  it('colors a negative DPR with the destructive class', async () => {
    // points_for 40 vs points_against 100 over 20 rounds -> DPR = -3.00
    mockUseTeamPlayerSeasonStats.mockReturnValue({
      stats: [statRow({ points_for: 40, points_against: 100 })],
      isLoading: false,
      isNotEnabled: false,
      error: null,
    });
  ...
  it('renders the bag mix bar and percentages from the bag counters', async () => {
    ...
    // The role=img label summarizes the whole bar for a11y.
    expect(screen.getByRole('img', { name: /Bag placement/i })).toBeInTheDocument();
  });

  it('falls back to a "not tracked" message when bags are null', async () => {
    mockUseTeamPlayerSeasonStats.mockReturnValue({
    ...
    expect(screen.getByText('Doug')).toBeInTheDocument();
    expect(screen.getByText('Bag placement not tracked')).toBeInTheDocument();
