import { Bolt, Scale, Star, Trophy } from 'lucide-react';
...
const StatsSummaryCards = ({ rankings }: StatsSummaryCardsProps) => {
  ...
  const getHighestWinPercentage = () => {
    ...
    const highest = rankings.reduce(
      (max, team) => (team.winPercentage > max.winPercentage ? team : max),
      rankings[0]
    );
    return {
  ...
  const getHighestSOS = () => {
    ...
    const highestSOS = rankings.reduce(
      (max, team) => ((team.sos || 0) > (max.sos || 0) ? team : max),
      rankings[0]
    );
    return {
  ...
  const getHighestPowerScore = () => {
    ...
    const highest = rankings.reduce(
      (max, team) => ((team.powerScore || 0) > (max.powerScore || 0) ? team : max),
      rankings[0]
    );
    return {
  ...
  return (
    <div className={cn('w-full mb-3 md:mb-4', listStyles.grid.stats)}>
      ...
      <SummaryCard
        ...
        title="Highest SOS"
        value={
          <span className={getValueClass('sos', Number(highestSOS.sos))}>{highestSOS.sos}</span>
        }
