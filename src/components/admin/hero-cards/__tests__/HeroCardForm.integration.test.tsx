...
describe('HeroCardForm integration', () => {
  ...
  it('supports create mode and blocks submit when required fields are empty', async () => {
    ...
    expect(save.closest('form')!.checkValidity()).toBe(false);
  ...
  it('supports edit mode and calls update', async () => {
    const onClose = vi.fn();
    const card = { id: 'c1', slug: 'old', title: 'Old', subtitle: null, body: null, cta_label: null, cta_url: null, background_color: 'bg-gradient-to-r from-blue-600 to-amber-500', text_color: 'text-white', accent_color: null, image_url: null, icon_name: null, is_visible: true, sort_order: 1, target_type: 'none', target_id: null, card_type: 'standard', metadata: {}, created_at: new Date().toISOString(), updated_at: new Date().toISOString() } as any;
    renderForm(<HeroCardForm card={card} onClose={onClose} />);
