      ...
      <div className="space-y-4">
        {matchPairs.map((pair) => (
          <MatchRow
            ...
            pair={pair}
            teams={teams || []}
            onUpdate={(updates) => updateMatchPair(pair.id, updates)}
            onRemove={() => removeMatchPair(pair.id)}
          />
        ))}
      </div>

      <div className="flex justify-between pt-4">
        <Button onClick={addMatchPair} variant="outline">
          + Add Another Match
        </Button>
