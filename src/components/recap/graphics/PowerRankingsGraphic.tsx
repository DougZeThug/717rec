...
/** One team's row on the graphic. */
const RankingRow: React.FC<{
  team: RecapTeamGrade;
  blurb: string;
  resolveLogo: (url: string | null) => string | null;
}> = ({ team, blurb, resolveLogo }) => {
  ...
  return (
    ...
    >
      <div
        style={{ display: 'flex', alignItems: 'center', gap: 16, height: RANKING_ROW.lineHeight }}
      >
        <span
          style={{
            width: 56,
        ...
        <span
          style={{ width: 62, fontFamily: recapFonts.numeric, fontSize: 22, color: movement.color }}
        >
          {movement.text}
        </span>

        <GraphicTeamMark teamName={team.teamName} logoSrc={resolveLogo(team.logoUrl)} size={44} />
        ...
        <span
          style={{
            ...
            // An unrated team gets a muted dash, never a letter it has not earned.
            backgroundColor: team.grade ? getGradeChartColor(team.grade) : recapColors.hairline,
          }}
        >
          {team.grade ?? '—'}
        </span>
        ...
        <span
          style={{ width: 86, textAlign: 'right', fontFamily: recapFonts.numeric, fontSize: 24 }}
        >
 ...
 *
 * The blurb is clamped to a single line by RankingRow because the row height is
 * fixed — nine rows have to fit the frame exactly. The full text always appears
 * on the edition's web page, which has no such limit.
 */
