...
import { BadgeConfig, BadgeType, ChampionshipBadgeMetadata, TeamBadgeEvent } from '@/types/badges';
...
export const badgeConfigs: Record<BadgeType, ExtendedBadgeConfig> = {
  ...
  // Fun badges (auto-awarded)
  ice_cold: {
    type: 'ice_cold',
    name: 'Ice Cold',
    description: 'Won 3 close matches in a row - nerves of steel',
  ...
  broom_crew: {
    type: 'broom_crew',
    name: 'Broom Crew',
    description: '3 sweeps in a row - cleaning house',
    icon: Brush,
  ...
  gatekeeper: {
    type: 'gatekeeper',
    name: 'Gatekeeper',
    description: 'Keeps beating teams ranked above them',
    icon: Shield,
...
export const getBadgeConfig = (
  ...
  if (badgeType === 'intermediate_champion' && badge?.metadata) {
    const metadata = badge.metadata as unknown as ChampionshipBadgeMetadata;
