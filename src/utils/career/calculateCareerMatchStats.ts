...
export const calculateCareerMatchStats = ({
  ...
  let career_match_wins =
    historicalStats?.reduce((sum, stat) => sum + (stat.match_wins || 0), 0) || 0;
  let career_match_losses =
    historicalStats?.reduce((sum, stat) => sum + (stat.match_losses || 0), 0) || 0;
  let career_game_wins =
    historicalStats?.reduce((sum, stat) => sum + (stat.game_wins || 0), 0) || 0;
  let career_game_losses =
    historicalStats?.reduce((sum, stat) => sum + (stat.game_losses || 0), 0) || 0;
