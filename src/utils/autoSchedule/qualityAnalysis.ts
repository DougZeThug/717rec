...
export function analyzeOpponentDiversity(pairings: TeamPairingMap): {
  ...
  const totalUniqueOpponents = Array.from(teamOpponents.values())
    .reduce((sum, opponents) => sum + opponents.size, 0);
  ...
  const diversityScore = maxPossibleOpponents > 0 
  ...
  allTeams.forEach(teamId => {
    ...
    if (blockOpponentSets.length > 1) {
      ...
      const hasOverlap = blockOpponentSets.some((set1, i) => 
        blockOpponentSets.some((set2, j) => 
          i !== j && Array.from(set1).some(opponent => set2.has(opponent))
        )
      );
...
export function calculateComprehensiveQualityMetrics(
  ...
  // Quality rating
  const qualityRating = averageCompatibilityScore >= 8 ? 'Excellent' :
                       averageCompatibilityScore >= 6 ? 'Good' :
                       averageCompatibilityScore >= 4 ? 'Fair' : 'Poor';
  ...
  const blocks = Object.keys(pairings);
  const blockAnalysis = blocks.length > 1 ? {
