...
function validatePairings(
  ...
  const incorrectTeams = teams.filter(team => 
...
function logFinalStatistics(
  ...
  finalPairings.forEach(pairing => {
    ...
    const tierKey = tier1 === tier2 ? `T${tier1}-T${tier2}` : `T${Math.min(tier1, tier2)}-T${Math.max(tier1, tier2)}`;
