...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.matches_archive WHERE id = v_match_id AND season_id = v_old_season_id) THEN
    RAISE EXCEPTION 'completed regular-season match was not copied to matches_archive';
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM public.team_details_archive
    WHERE season_id = v_old_season_id
      AND team_id = v_team1_id
      AND wins >= 1
      AND game_wins >= 2
