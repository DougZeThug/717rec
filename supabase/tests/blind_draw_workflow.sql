...
DO $$
BEGIN
  IF NOT has_table_privilege('anon', 'public.blind_draw_signups', 'INSERT') THEN
    RAISE EXCEPTION 'anon is missing INSERT on blind_draw_signups';
  END IF;
  IF NOT has_function_privilege('anon', 'public.get_blind_draw_signup_count(date)', 'EXECUTE') THEN
    RAISE EXCEPTION 'anon is missing EXECUTE on get_blind_draw_signup_count(date)';
  END IF;
  IF NOT has_table_privilege('authenticated', 'public.blind_draw_signups', 'SELECT') THEN
    RAISE EXCEPTION 'authenticated is missing SELECT on blind_draw_signups';
  END IF;
  IF NOT has_table_privilege('authenticated', 'public.blind_draw_signups', 'INSERT') THEN
    RAISE EXCEPTION 'authenticated is missing INSERT on blind_draw_signups';
  END IF;
  IF NOT has_table_privilege('authenticated', 'public.blind_draw_signups', 'DELETE') THEN
    RAISE EXCEPTION 'authenticated is missing DELETE on blind_draw_signups';
  END IF;
  IF NOT has_table_privilege('authenticated', 'public.blind_draw_settings', 'UPDATE') THEN
    RAISE EXCEPTION 'authenticated is missing UPDATE on blind_draw_settings';
  END IF;
  IF NOT has_function_privilege('authenticated', 'public.get_blind_draw_signup_count(date)', 'EXECUTE') THEN
    RAISE EXCEPTION 'authenticated is missing EXECUTE on get_blind_draw_signup_count(date)';
  END IF;
END $$;
...
BEGIN
  ...
  EXCEPTION WHEN insufficient_privilege THEN
    NULL; -- expected when table grants, not just RLS, prevent public visibility
  END;
