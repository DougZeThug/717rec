-- CRITICAL SECURITY FIX: Admin Privilege Escalation Prevention (Fixed)
...
FOR UPDATE
USING (auth.uid() = id)
WITH CHECK (auth.uid() = id AND (OLD.is_admin IS NOT DISTINCT FROM NEW.is_admin));

-- Only existing admins can modify admin status
CREATE POLICY "Only admins can modify admin privileges"
ON public.profiles
FOR UPDATE
USING (current_user_is_admin())
