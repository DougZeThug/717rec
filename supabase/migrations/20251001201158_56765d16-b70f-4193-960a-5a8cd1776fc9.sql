-- Update power score formula weights from 40/40/20 to 40/45/15
-- This increases the weight of Strength of Schedule from 40% to 45%
-- and decreases Weighted Game Win Percentage from 20% to 15%
-- This better rewards teams that play difficult schedules
...
SELECT 
    ...
    -- UPDATED: Return NULL for teams with no completed matches
...
LEFT JOIN (
    -- UPDATED: 40/45/15 POWER SCORE FORMULA - return NULL for teams with no matches
    SELECT 
        t.id as team_id,
        CASE 
            WHEN COUNT(m.*) = 0 THEN NULL  -- CHANGED: Return NULL instead of 50.0 for teams with no matches
            ELSE (
                -- 40% Weighted Match Win Percentage (CORRECTED)
                (COALESCE(weighted_match_stats.weighted_win_percentage, 0) * 40.0) +
                -- 45% Strength of Schedule (UPDATED from 40%)
                (COALESCE(sos_component.sos, 0.5) * 45.0) +
                -- 15% Weighted Game Win Percentage (UPDATED from 20%)
