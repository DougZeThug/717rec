...
--
-- The first branch of its UNION ALL chain selects 15 expressions (a stray
-- leading bracket_id) while the INSERT lists 14 columns and every other
-- branch has 14, so "each UNION query must have the same number of columns"
-- always errored and the file's transaction aborted.
-- 20250905193107_8300fa7e is the working rewrite of this backfill (plain
-- VALUES lists). Original text kept below for history.
