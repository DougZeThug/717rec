...
BEGIN
  ...
  LOOP
    -- ============================================================
    -- Set bracket-aware division_name for all teams in this bracket
    -- ============================================================
    IF v_bracket.sibling_count = 1 THEN
      -- Only 1 bracket for this display_division -> use display_division as-is
      v_display_div := v_bracket.display_div;
    ELSE
      -- Multiple brackets -> number them (e.g. "Intermediate 1", "Intermediate 2")
      v_display_div := v_bracket.display_div || ' ' || v_bracket.bracket_num;
    END IF;

    -- Update division_name for all participants in this bracket
    UPDATE public.team_season_stats tss
    ...
    -- ============================================================
    -- Assign ranks 4+ based on elimination round (with tie handling)
    -- ============================================================
    v_current_rank := 4;
  ...
  -- Deactivate all non-permanent badges.
  -- Permanent badges (never wiped): cool_fun_team
  UPDATE public.team_badge_events
  SET is_active = false
  WHERE is_active = true
    AND badge_type NOT IN ('cool_fun_team');

  -- Award champion badges for each bracket/division
  FOR v_bracket IN
