...
SELECT 
    ...
    -- Return NULL for teams with no completed matches
    power_calc.power_score AS power_score
FROM teams t
LEFT JOIN divisions d ON t.division_id = d.id
LEFT JOIN v_team_match_stats stats ON t.id = stats.team_id
-- ADDED: Filter out teams that have opted out of the current active season
LEFT JOIN team_season_opt_out tso ON t.id = tso.team_id 
    AND tso.season_id = (SELECT id FROM seasons WHERE is_active = true LIMIT 1)
LEFT JOIN (
...
LEFT JOIN (
    -- 40/40/20 POWER SCORE FORMULA - return NULL for teams with no matches
    SELECT 
        t.id as team_id,
        CASE 
...
-- ADDED: Exclude teams that have opted out of the current active season
