-- ============================================================
-- Overlapping Season Playoffs
--
-- Adds a second "active" axis on public.seasons so an old season's
-- playoff bracket can keep running after a new regular season starts.
--
-- Contents:
--   PART 1: playoffs_active column + single-instance trigger
--   PART 2: partial_archive_season(p_season_id)
--   PART 3: finalize_playoffs(p_season_id, champ, runner_up, third)
--   PART 4: activate_season_with_partial_archive(p_new_season_id)
-- ============================================================

-- ============================================================
-- PART 1: Column + trigger
-- ============================================================
ALTER TABLE public.seasons
  ADD COLUMN IF NOT EXISTS playoffs_active boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_seasons_playoffs_active
  ON public.seasons(playoffs_active);

CREATE OR REPLACE FUNCTION public.ensure_single_playoffs_active_season()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = 'pg_catalog', 'public'
AS $$
BEGIN
  IF NEW.playoffs_active = true
     AND (OLD.playoffs_active IS NULL OR OLD.playoffs_active = false) THEN
    UPDATE public.seasons
      SET playoffs_active = false
      WHERE id != NEW.id AND playoffs_active = true;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS ensure_single_playoffs_active_season_trg ON public.seasons;
CREATE TRIGGER ensure_single_playoffs_active_season_trg
  BEFORE INSERT OR UPDATE ON public.seasons
  FOR EACH ROW EXECUTE FUNCTION public.ensure_single_playoffs_active_season();

-- ============================================================
-- PART 2: partial_archive_season
--
-- Archives regular-season matches, resets team counters, and flips
-- the season into "playoffs only" mode (is_active=false,
-- is_archived=false, playoffs_active=true). Does NOT detect
-- champions, snapshot team details, or rotate badges — those run
-- later in finalize_playoffs.
-- ============================================================
...
BEGIN
  ...
  -- Refresh stats so anything riding on team_season_stats is current
  -- before we move the source rows to matches_archive.
  PERFORM public.upsert_team_season_stats();

  -- Archive regular-season matches (mirrors archive_season STEP 5)
  ...
  -- Reset teams table counters so the next season starts clean
  -- (mirrors archive_season STEP 5b)
  UPDATE public.teams
  SET wins = 0, losses = 0, game_wins = 0, game_losses = 0;

  -- Flip the season into "playoffs only" mode
  UPDATE public.seasons
  SET
    is_active = false,
...
-- ============================================================
-- PART 3: finalize_playoffs
--
-- Completes the archival of a season that was previously partial-
-- archived. Runs the parts of archive_season that were skipped:
--   - refresh stats (picks up final playoff results)
--   - division-name sync
--   - champion / runner-up / playoff_rank detection
--   - team_details_archive snapshot
--   - badge rotation
-- Then flips is_archived=true, playoffs_active=false.
-- Does NOT re-archive regular matches — they are already gone.
-- ============================================================
...
BEGIN
  ...
  -- STEP 1: Refresh stats (picks up final playoff results via v_team_season_agg)
  PERFORM public.upsert_team_season_stats();

  -- STEP 2: Initial division-name sync (skip "Hidden" stomp)
  ...
  -- STEP 3: Auto-detect playoff finishing positions
  ...
  -- STEP 6 (renamed): Close out the season record
  UPDATE public.seasons
  SET
    is_active = false,
    is_archived = true,
...
-- ============================================================
-- PART 4: activate_season_with_partial_archive
--
-- "Overlap" flip: partial-archives the current active season (if
-- any), then activates the target season. New regular matches
-- schedule on the new season; the old season's playoff bracket
-- stays live and its results still contribute to its stats.
-- ============================================================
