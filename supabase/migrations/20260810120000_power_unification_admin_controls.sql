...
--
-- The unification recalculates every historical power score, so Career
-- Rankings re-order. The 20260809121000 migration snapshots the old numbers
-- into team_season_stats_pre_unification / team_details_power_pre_unification,
-- but those tables are RLS-locked with no policies (nothing can read them) and
-- undoing the change meant hand-written SQL.
--
-- This migration adds five admin-only functions the "Power Score Review"
-- admin tab is built on:
--
--   admin_power_unification_status()            where things stand, from the catalog
--   admin_get_pre_unification_season_stats()    read the season backup
--   admin_get_pre_unification_team_power()      read the team-power backup
--   admin_revert_power_score_unification()      one-call revert
--   admin_reapply_power_score_unification()     one-call re-apply
--
-- Revert does NOT copy backup rows into team_season_stats. It restores the
-- pre-unification view definitions and re-runs upsert_team_season_stats(),
-- which re-derives every season from match history under the old formula.
-- Games played after the backup was taken are therefore kept, not lost.
-- The restored definitions are verbatim copies of the ones the unification
-- replaced: v_team_match_stats from the baseline, v_team_details and its
-- dependents from 20251002122723, v_team_season_agg from 20260408173910 and
-- get_season_team_power_scores (40/40/20) from 20260330152902. Re-apply is
-- the mirror image, restoring the definitions from 20260809122000/124000/125000.
--
-- Everything is plpgsql on purpose: plpgsql resolves table and view names at
-- run time, so this file applies cleanly even on a database where the
-- 20260809* migrations have not run yet (the functions then report
-- 'not_applied' / raise a friendly error instead of failing to install).
--
-- All functions are SECURITY DEFINER with a pinned search_path and refuse
-- non-admin callers ('Admin access required'), matching reconcile_team_counters.

-- ---------------------------------------------------------------------------
-- 1. Status. Derived from the live catalog rather than a marker table: after
--    the unification both v_team_season_agg and v_team_details call
--    public.power_score_100(); before it (and after a revert) neither does.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  ELSIF v_agg_unified OR v_details_unified THEN
    -- One view unified, one not: an interrupted revert/reapply. Should not
    -- happen (both run in one transaction) but is worth surfacing loudly.
    status := 'partial';
  ...
  IF v_backup_exists THEN
    -- Dynamic SQL: the backup tables may not exist, and a static reference
    -- would make this function unusable on a pre-unification database.
...
-- ---------------------------------------------------------------------------
-- 2. Backup readers. The backup tables are RLS-locked with no policies, so
--    these definer-rights functions are the only way the app can read them.
--    On a database where the unification has not run, the tables do not exist
--    and the functions raise undefined_table (42P01), which the frontend maps
--    to its "not applied yet" state.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Revert. Restores the pre-unification view definitions and the 40/40/20
--    get_season_team_power_scores, then re-runs upsert_team_season_stats()
--    so team_season_stats is re-derived under the old formula (keeping games
--    played after the backup). Leaves the unification's shared objects
--    (power_score_100, v_power_score_*) and the backup tables in place —
--    re-apply needs both.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Same take-down order as 20260809124000: v_team_details depends on
  ...
  -- v_team_match_stats: baseline definition — public.matches only, no
  -- playoffs. (security_invoker is kept on: it changes nothing about the
  -- data and avoids reintroducing a definer-rights view.)
  EXECUTE $def$
  ...
  -- v_team_season_agg: 20260408173910 definition — playoff rows counted with
  -- no bye exclusion, plain (unweighted) win rate, 0.40/0.45/0.15 on 0-1.
  EXECUTE $def$
  ...
  -- public.matches only. Same signature as the canonical version, so
...
-- ---------------------------------------------------------------------------
-- 4. Re-apply. Mirror image of revert: restores the canonical definitions
--    from 20260809122000/124000/125000 and re-runs upsert_team_season_stats().
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- The canonical definitions read the shared objects created by the
  -- 20260809* migrations; without them there is nothing to re-apply.
  ...
  -- v_team_match_stats: 20260809124000 definition — records over the shared
  -- current-season match source, playoff games included.
  EXECUTE $def$
    CREATE VIEW public.v_team_match_stats
    WITH (security_invoker = on)
    AS
  ...
  -- replaces the inline power_calc subquery.
  EXECUTE $def$
    CREATE VIEW public.v_team_details
    WITH (security_invoker = on)
    AS
  ...
  -- v_team_season_agg: 20260809125000 definition — canonical formula and
  -- match source, byes excluded, power_score kept on the 0-1 storage scale.
  EXECUTE $def$
    CREATE VIEW public.v_team_season_agg
    WITH (security_invoker = on)
    AS
...
-- ---------------------------------------------------------------------------
-- 5. Shared helper for the two directions above. v_team_details_with_season,
--    v_team_power_scores and v_team_strength_of_schedule read v_team_details /
--    v_team_match_stats by name and are identical before and after the
--    unification, so both revert and re-apply rebuild them the same way after
--    the CASCADE drops. Definitions verbatim from 20251002122723 and the
--    baseline. Not exposed over the API: only the admin functions call it.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Dropped by the v_team_match_stats cascade. Recreated only when its own
  -- dependency exists; nothing in the app reads it.
