-- Fix search_path for SECURITY DEFINER badge functions
-- Adding pg_catalog before public to prevent search_path manipulation attacks
-- where malicious users could shadow built-in PostgreSQL functions

-- 1. award_broom_crew_badge
CREATE OR REPLACE FUNCTION public.award_broom_crew_badge(p_team_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'pg_catalog', 'public'  -- Fixed: added pg_catalog
AS $function$
DECLARE
...
BEGIN
  ...
  FROM public.matches m
  JOIN public.v_team_power_scores opp ON opp.team_id = 
    CASE WHEN m.team1_id = p_team_id THEN m.team2_id ELSE m.team1_id END
  WHERE (m.team1_id = p_team_id OR m.team2_id = p_team_id)
    AND m.winner_id = p_team_id
    AND m.iscompleted = true
