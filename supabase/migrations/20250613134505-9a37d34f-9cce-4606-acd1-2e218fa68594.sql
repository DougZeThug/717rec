...
-- Update v_team_details view to implement the correct 40/40/20 power score formula
...
LEFT JOIN (
    -- CORRECT 40/40/20 POWER SCORE FORMULA (0-100 scale)
    SELECT 
        t.id as team_id,
        CASE 
    ...
    LEFT JOIN matches m ON (t.id = m.team1_id OR t.id = m.team2_id) AND m.iscompleted = true
    LEFT JOIN (
        -- Weighted match win percentage for power score
        SELECT 
            t.id as team_id,
            CASE 
                WHEN COUNT(m.*) = 0 THEN 0
    ...
    LEFT JOIN (
        -- Weighted game win percentage for power score
        SELECT 
            t.id as team_id,
            CASE 
                WHEN SUM(CASE WHEN m.team1_id = t.id THEN m.team1_game_wins + COALESCE(m.team2_game_wins, 0)
