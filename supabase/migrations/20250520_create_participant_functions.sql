...
--
-- get_participants declares an unquoted "position INT" output column —
-- POSITION is a reserved word, so the file always died with a syntax error
-- and neither function was ever created from it. The live project got these
-- helpers from the SQL editor instead, with different signatures: live
-- insert_participant takes p_team_position (types.ts) and is (re)created
-- in-chain by 20251225204207; live get_participants returns a team_position
-- column and is provided by the baseline migration.
