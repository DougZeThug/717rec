-- reopen_live_match: also reopen a match that completed without a winner.
--
-- A tie is stored as iscompleted = true with winner_id = NULL (see
-- mark_match_as_tie). reopen_live_match read only winner_id, so it returned
-- false for those matches and cleared nothing -- iscompleted stayed true.
-- finalize_live_match then refused with 'already_completed', so the one-press
-- "Reopen & re-save result" button was a guaranteed no-op and a drifted tie
-- could not be corrected from the app at all. The dashboard's drift card lists
-- those matches and tells the admin to press exactly that button.
--
-- The new branch clears the completion so the match is editable again. It
-- changes no team records on purpose: mark_match_as_tie already removed the
-- win, the loss and both game counts, so there is nothing left to undo.
-- Reopening also drops the tie_confirmed_at / tie_confirmed_by stamps, since a
-- match that is open again is no longer a confirmed tie.
--
-- The winner path below is unchanged.
