...
export function getClientIp(req: Request): string {
  ...
  // behavior. Ignore the trusted proxy suffix and use the client hop immediately
  // to its left so unrelated clients do not share the proxy's rate-limit bucket.
  return hops[hops.length - trustedProxyHops - 1];
}
